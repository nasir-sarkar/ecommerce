...
function fmtDate(iso) {
  ...
  const h = d.getHours()
  return `${String(d.getDate()).padStart(2,'0')}-${String(d.getMonth()+1).padStart(2,'0')}-${d.getFullYear()} ${String(h).padStart(2,'0')}:${String(d.getMinutes()).padStart(2,'0')} ${h >= 12 ? 'PM' : 'AM'}`
}
...
// Seller panel theme: #2E294E
const THEME = '#2E294E'

export default function ViewOrder_Seller() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/orders/seller`, {
      ...
      .then(d => {
        if (d.success) {
          const found = (d.data || []).find(o => o._id === id)
          if (found) {
            setOrder(found)
            setDeliveryStatus(found.status || 'pending')
          } else {
  ...
  // Seller: pending - confirmed - delivered
  ...
  return (
    <div style={{ padding: '0 0 32px' }}>
      <div className="card" style={{ borderRadius: 8, background: '#fff', border: '1px solid #f1f1f4', boxShadow: '0px 6px 14px rgba(35, 39, 52, 0.04)', margin: '0' }}>
        ...
        {/* Card Header */}
        <div className="card-header" style={{ padding: '16px 20px', borderBottom: '1px solid #f1f1f4', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <button
              type="button"
              onClick={() => navigate('/seller/orders')}
              style={{ background: '#f5f5f7', border: 'none', borderRadius: 6, width: 32, height: 32, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#2E294E' }}
            >
              <IconBack />
            </button>
            <h1 style={{ fontSize: 16, fontWeight: 600, margin: 0, color: '#2E294E' }}>Order Details</h1>
          </div>

          {/* Delivery action button — seller can confirm then mark delivered */}
          <div>
            {deliveryStatus === 'pending' && (
              <button type="button" disabled={saving} onClick={() => handleDeliveryAction('confirmed')}
                style={{ background: '#19c553', color: '#fff', border: 'none', borderRadius: 6, padding: '8px 20px', fontSize: 13, fontWeight: 700, cursor: saving ? 'not-allowed' : 'pointer', opacity: saving ? 0.7 : 1, boxShadow: '0 3px 10px rgba(25,197,83,0.3)', display: 'inline-flex', alignItems: 'center', gap: 6 }}>
            ...
            {deliveryStatus === 'confirmed' && (
              <button type="button" disabled={saving} onClick={() => handleDeliveryAction('delivered')}
                style={{ background: THEME, color: '#fff', border: 'none', borderRadius: 6, padding: '8px 20px', fontSize: 13, fontWeight: 700, cursor: saving ? 'not-allowed' : 'pointer', opacity: saving ? 0.7 : 1, boxShadow: `0 3px 10px rgba(46,41,78,0.3)`, display: 'inline-flex', alignItems: 'center', gap: 6 }}>
            ...
            {deliveryStatus === 'delivered' && (
              <span style={{ background: '#e8fff3', color: '#19c553', border: '1px solid #c6f6d5', borderRadius: 6, padding: '8px 20px', fontSize: 13, fontWeight: 700, display: 'inline-block' }}>✓ Delivered</span>
            )}
          </div>
        </div>
        ...
        <div style={{ padding: '20px' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16, marginBottom: 16 }}>
            <div style={{ flex: 1, minWidth: 220 }}>
              <address style={{ fontStyle: 'normal', fontSize: 13, lineHeight: 1.7 }}>
                <strong style={{ color: '#2E294E' }}>{customerName}</strong><br />
                {customer.email && <>{customer.email}<br /></>}
                {customer.phone && <>{customer.phone}<br /></>}
                {[customer.address, customer.city, customer.state, customer.country].filter(Boolean).join(', ')}{customer.postal_code && <>, {customer.postal_code}</>}
              </address>
              <div style={{ fontSize: 13, marginTop: 8 }}>
                <strong style={{ color: '#2E294E' }}>Sold by: </strong>
                {sellerName}
              </div>
            </div>
            ...
            <div style={{ minWidth: 280 }}>
              <table style={{ width: '100%', fontSize: 13 }}>
                <tbody>
                  <tr>
                    <td style={{ padding: '4px 8px 4px 0', fontWeight: 600, color: '#2E294E' }}>Order #</td>
                    <td style={{ padding: '4px 0', textAlign: 'right', color: THEME, fontWeight: 600 }}>{fmtCode(order)}</td>
                  </tr>
                  <tr>
                    <td style={{ padding: '4px 8px 4px 0', fontWeight: 600, color: '#2E294E' }}>Order status</td>
                  ...
                  <tr>
                    <td style={{ padding: '4px 8px 4px 0', fontWeight: 600, color: '#2E294E' }}>Order date</td>
                    <td style={{ padding: '4px 0', textAlign: 'right' }}>{fmtDate(order.createdAt)}</td>
                  </tr>
                  <tr>
                    <td style={{ padding: '4px 8px 4px 0', fontWeight: 600, color: '#2E294E' }}>Total amount</td>
                    <td style={{ padding: '4px 0', textAlign: 'right' }}>{fmt(grandTotal)}</td>
                  </tr>
                  <tr>
                    <td style={{ padding: '4px 8px 4px 0', fontWeight: 600, color: '#2E294E' }}>Payment method</td>
                    <td style={{ padding: '4px 0', textAlign: 'right' }}>
                      {(order.paymentMethod || '').replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase())}
                    </td>
                  </tr>
                  <tr>
                    <td style={{ padding: '4px 8px 4px 0', fontWeight: 600, color: '#2E294E' }}>Payment status</td>
                    <td style={{ padding: '4px 0', textAlign: 'right' }}>
                      {(order.paymentStatus || 'unpaid').toLowerCase() === 'paid'
                        ? <span style={{ background: '#28a745', color: '#fff', fontSize: 11, fontWeight: 600, padding: '2px 8px', borderRadius: 3 }}>Paid</span>
                        : <span style={{ background: '#f1416c', color: '#fff', fontSize: 11, fontWeight: 600, padding: '2px 8px', borderRadius: 3 }}>Un-Paid</span>
                      }
                    </td>
                  </tr>
                </tbody>
          ...
          <div style={{ overflowX: 'auto', marginBottom: 20 }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', border: '1px solid #dee2e6', fontSize: 13 }}>
              <thead>
                <tr style={{ background: 'rgba(46,41,78,0.06)' }}>
                  <th style={thStyle}>#</th>
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td style={{ padding: '4px 0' }}>
                    <span style={{ color: '#2E294E', fontSize: 16, fontWeight: 600 }}>{fmt(grandTotal)}</span>
                  </td>
                </tr>
              </tbody>
            </table>
