...
function fmtCode(order) {
  // Generate order code from createdAt + last 8 chars of id
...
function ToolbarRow({ search, onSearch, deliveryFilter, onDeliveryFilter, paymentFilter, onPaymentFilter }) {
  ...
  const paymentOptions  = ['All', 'Paid', 'Un-Paid']
    ...
    <div className="px-[25px] pt-[16px] pb-[16px] flex flex-wrap gap-[8px] items-center">
      ...
      <div className="relative w-[200px]">
        <button type="button" onClick={() => setPaymentOpen(o => !o)}
          className="w-full bg-[#f5f5f7] border border-[#f1f1f4] rounded-[6px] h-[38px] px-[14px] flex items-center justify-between text-[13px] text-[#9da3ae]">
          <span>{paymentFilter === 'All' ? 'Filter by Payment Status' : paymentFilter}</span>
          <CaretIcon />
        </button>
        {paymentOpen && (
          <div className="absolute right-0 top-full mt-1 z-30 w-full bg-white rounded-[6px] border border-[#f1f1f4] shadow-[0px_6px_14px_rgba(35,39,52,0.12)] py-2">
            {paymentOptions.map(label => (
              <button key={label} type="button" onClick={() => { onPaymentFilter(label); setPaymentOpen(false) }}
                className={`block w-full text-left px-[12px] py-[6px] text-[13px] hover:bg-[#f1fafd] ${paymentFilter === label ? 'text-[#009ef7] font-semibold' : 'text-[#232734]'}`}>
                {label}
              </button>
            ))}
          </div>
...
export default function AllOrders_Admin() {
  ...
  const filtered = orders.filter(o => {
    ...
    if (activeTab === 'In-House') {
      const hasInhouse = o.items?.some(i => !i.seller || i.seller === '')
      if (!hasInhouse) return false
    ...
    if (paymentFilter !== 'All') {
      ...
      if (paymentFilter === 'Un-Paid' && ps !== 'unpaid') return false
    }
    // Search
    if (search.trim()) {
  ...
  return (
    <>
      <h1 className="text-[20px] leading-[28px] font-bold text-[#232734] mb-[16px]">All Orders</h1>
      <Card>
