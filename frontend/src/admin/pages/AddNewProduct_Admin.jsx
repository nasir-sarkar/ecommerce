...
function ImageUpload({ label, note, value, onChange, fieldName }) {
...
export default function AddNewProduct_Admin() {
  // DB data
  ...
  const submit = async (publishedVal) => {
    ...
    try {
      ...
      // Reset form
      setTitle(''); setCategory(''); setSubCategory(''); setBrand(''); setDescription('')
      setTags([]); setImage(''); setImage2(''); setImage3('')
      setPrice('0'); setStock('0'); setDiscount('0')
    } catch (err) {
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between mb-[16px] flex-wrap gap-2">
        <h1 className="text-[20px] leading-[28px] font-bold text-[#232734] m-0">Add New Product</h1>
